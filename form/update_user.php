...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
    ...
    if ($update_stmt->execute()) {
        ...
        echo "<div class='alert alert-danger'>Error updating user details.</div>";
    }
}
...
if (isset($_POST['update_family'])) {
    foreach ($_POST['family'] as $family_id => $family_data) {
        ...
        if (!$family_update_stmt->execute()) {
            echo "<div class='alert alert-danger'>Error updating family member details.</div>";
        }
    }
...
if (isset($_POST['update_seminars'])) {
    foreach ($_POST['seminars'] as $seminar_id => $seminar_data) {
        $seminar_update_sql = "UPDATE seminars_members SET 
        ...
        if (!$seminar_update_stmt->execute()) {
            echo "<div class='alert alert-danger'>Error updating seminar details.</div>";
        }
    }
...
<body>
    <div class="container mt-5">
                    ...
                    <table class="table table-bordered" id="familyTable">
                        ...
                        <tbody>
                            <?php foreach ($family_members as $member): ?>
                                <tr>
                                    <td><input type="text" class="form-control" name="family[<?php echo $member['id']; ?>][name]" value="<?php echo htmlspecialchars($member['name'] ?? ''); ?>" required></td>
                                    <td><input type="text" class="form-control" name="family[<?php echo $member['id']; ?>][relationship]" value="<?php echo htmlspecialchars($member['relationship'] ?? ''); ?>" required></td>
                                    <td><input type="number" class="form-control" name="family[<?php echo $member['id']; ?>][age]" value="<?php echo htmlspecialchars($member['age'] ?? ''); ?>" required></td>
                                    <td><input type="date" class="form-control" name="family[<?php echo $member['id']; ?>][birthday]" value="<?php echo htmlspecialchars($member['birthday'] ?? ''); ?>" required></td>
                                    <td><input type="text" class="form-control" name="family[<?php echo $member['id']; ?>][occupation]" value="<?php echo htmlspecialchars($member['occupation'] ?? ''); ?>" required></td>
                                    <td><button type="button" class="btn btn-danger removeRow">Remove</button></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                    <button type="button" class="btn btn-success" id="addFamilyRow">Add Family Member</button>
        ...
        <div class="card">
            <div class="card-header">Edit Seminar Details</div>
            <div class="card-body">
                <form method="POST" action="">
                    <table class="table table-bordered" id="seminarTable">
                        ...
                        <tbody>
                            <?php foreach ($seminars as $seminar): ?>
                                <tr>
                                    <td><input type="text" class="form-control" name="seminars[<?php echo $seminar['id']; ?>][title]" value="<?php echo htmlspecialchars($seminar['title'] ?? ''); ?>"></td>
                                    <td><input type="date" class="form-control" name="seminars[<?php echo $seminar['id']; ?>][date]" value="<?php echo htmlspecialchars($seminar['date'] ?? ''); ?>"></td>
                                    <td><input type="text" class="form-control" name="seminars[<?php echo $seminar['id']; ?>][organizer]" value="<?php echo htmlspecialchars($seminar['organizer'] ?? ''); ?>"></td>
                                    <td><button type="button" class="btn btn-danger removeRow">Remove</button></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                    <button type="button" class="btn btn-success" id="addSeminarRow">Add Seminar</button>
        ...
        <script>
            document.getElementById ('addFamilyRow').addEventListener('click', function() {
                ...
                newRow.innerHTML = `
                    <td><input type="text" class="form-control" name="family[][name]" required></td>
                    <td><input type="text" class="form-control" name="family[][relationship]" required></td>
                    <td><input type="number" class="form-control" name="family[][age]" required></td>
                    <td><input type="date" class="form-control" name="family[][birthday]" required></td>
                    <td><input type="text" class="form-control" name="family[][occupation]" required></td>
                    <td><button type="button" class="btn btn-danger removeRow">Remove</button></td>
                `;
            });
            ...
            document.getElementById('addSeminarRow').addEventListener('click', function() {
                ...
                const newRow = table.insertRow();
                newRow.innerHTML = `
                    <td><input type="text" class="form-control" name="seminars[][title]"></td>
                    <td><input type="date" class="form-control" name="seminars[][date]"></td>
                    <td><input type="text" class="form-control" name="seminars[][organizer]"></td>
                    <td><button type="button" class="btn btn-danger removeRow">Remove</button></td>
                `;
            });
        ...
        <br>
        <a href="view_details.php" class="btn btn-success">Go back</a>
    </div>
</body>
</html>